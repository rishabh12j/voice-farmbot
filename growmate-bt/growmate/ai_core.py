"""
GrowMate AI Core v4 -- Multi-Intent + FarmBot Live + Mixed Commands

Upgrades from v3:
1. Multi-intent classifier: "water tomatoes and go home" -> [water, go_home]
2. FarmBot live lookup: trees query FarmBot API for real plant coords
3. Mixed commands: robot + general in any order, all in one BT
4. check_moisture vs check_sensor distinction fixed
"""
...
"when should I plant basil" -> {{"intents": [{{"action":"general_question","target":"basil","question":"When should I plant basil in Ireland?","response":"Let me look that up."}}]}}
...
"what vegetables grow well in spring" -> {{"intents": [{{"action":"general_question","target":null,"question":"What vegetables grow well in spring in Ireland?","response":"Let me find out."}}]}}
"water the tomatoes and tell me about basil" -> {{"intents": [{{"action":"water","target":"tomatoes","question":null,"response":"Watering the tomatoes."}},{{"action":"general_question","target":"basil","question":"Tell me about growing basil in Ireland.","response":"And here is some basil advice."}}]}}
    ...
    def _tree_general(self, question, resp):
        ...
        return {"type": "sequence", "label": "General question", "children": [
            ...
            {"type": "function_call", "name": "fetch_weather",
             "params": {"location": "Maynooth", "days": 3}, "store_as": "weather"},
