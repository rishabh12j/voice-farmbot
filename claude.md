...
1. Make the FastAPI web app work end-to-end on my machine (this is the current focus).
...
- `pyaudio` does not install cleanly on Windows via `pip` in most cases. Do **not** suggest `pip install pyaudio` unless I ask — the web app doesn't need it. Browser mic uploads WAV to the FastAPI `/voice` endpoint.
...
├── demo/                            ← Demo planning: presentations, questionnaire, interim report PDF
├── growmate-bt/                     ← Legacy standalone thesis code (read-only baseline)
│   ├── evaluate_bt.py               (29-utterance eval corpus — NOT yet ported to ROS2 package)
│   └── growmate/                    (original ai_core, bt_engine, robot_controller, speech, main.py)
└── src/
    ├── camera_handler/              ┐
    ├── farmbot_bringup/             │
    ...
    └── growmate_voice/              ← My package. Edit anything here.
        ...
        ├── requirements.txt         (pip deps for this env — FastAPI stack)
        ├── requirements-pi.txt      (lean Pi runtime — no torch, no kokoro)
        ...
        ├── config/
        │   └── farmbot.yaml         (garden, plants, commands, safety, schedule)
        ├── launch/
        ...
        └── growmate_voice/
            ...
            ├── bt_bridge.py         (dual-source loader for workbench; constructs trees without executing)
            ├── ros2_publisher.py    (publishes to keyboard_topic; sim mode on Windows)
            ├── speech.py            (STT/TTS wrappers — delegates to edgespeech/ backends)
            ├── app.py               (FastAPI web app — THIS is what I run on Windows, port 7860)
            ├── stt_test.py          (standalone STT/TTS/BT workbench — port 7870, no robot control)
            ├── scheduler.py         (daily watering daemon: fires P_4 once per day at scheduled time)
            ├── history.py           (persistent command/action history as JSON log)
            ├── logger.py            (structured logging to ~/.growmate_voice/growmate_voice.log)
            ├── voice_controller_node.py  (headless ROS2 node — Pi only)
            ├── cli.py               (desktop CLI, forces --no-ros2)
            └── edgespeech/          (pluggable STT and TTS backends)
                ├── audio_utils.py   (WAV codec, sample-rate management)
                ├── command_map.py   (fast pattern-based command matching — LLM fallback)
                ├── stt/             (whisper_backend.py, vosk_backend.py, moonshine_backend.py)
                └── tts/             (piper_backend.py, kokoro_backend.py)
```

**On Windows, the primary entry points are:**
- `voice_app` / `python -m growmate_voice.app` — main web UI (jog + voice, port 7860)
- `voice_workbench` / `python -m growmate_voice.stt_test` — STT/TTS/BT experimentation (port 7870, no robot)
- `voice_scheduler` / `python -m growmate_voice.scheduler` — daily watering daemon (Pi-targeted but runnable on Windows in sim)
...
### Running the FastAPI web app (simulation mode — no real robot)
...
### Running the STT/TTS/BT workbench (no robot, experimentation only)

```powershell
conda activate moderation
cd C:\path\to\voice-farmbot\src\growmate_voice
python -m growmate_voice.stt_test
```

Opens at `http://localhost:7870`. Shows STT backend picker, command match debug, ASCII BT view, and TTS test panel. No robot commands are sent.
...
### Running the daily watering scheduler (Pi-targeted, sim-safe on Windows)

```powershell
cd C:\path\to\voice-farmbot\src\growmate_voice
python -m growmate_voice.scheduler --no-ros2
```

Fires `P_4` (water all) once per day at the time in `config/farmbot.yaml → schedule.watering_time` (default 08:00). Persistent state in `~/.growmate_voice/last_watered.txt` prevents double-firing on reboot.
...
**Fixing imports and paths.** When I move a module or add a new one, imports break. You should read the whole file (not just the error) before proposing a fix, because some modules are deliberately lazy-imported (`rclpy`, `faster_whisper`, `vosk`, `piper`) and should stay that way — see §7.
...
**Evaluation.** The 29-utterance corpus lives in `growmate-bt/evaluate_bt.py` (the legacy standalone tree). If I ask to "run the eval," we need to port it across and update imports to the ROS2 package structure.

**The FastAPI web UI.** Layout tweaks, fixing the confirmation flow, making the tree visualisation clearer. `app.py` is the only file for this (1088 lines as of now).

**STT/TTS backend switching.** Backend selection is in `edgespeech/stt/` and `edgespeech/tts/`. The `app.py` and `stt_test.py` pick a backend at startup via `--stt` and `--tts` flags. `speech.py` is the older thin wrapper — new code should go through `edgespeech/` instead.

**Adding pattern-based command shortcuts.** `edgespeech/command_map.py` maps transcribed text to FarmBot commands without calling the LLM. Useful for high-frequency commands that don't need BT construction. Extend this when a command is too simple to justify an LLM round-trip.
...
3. **Emergency stop never goes through the LLM.** The phrases `stop`, `halt`, `emergency`, `emergency stop`, `freeze`, `abort` are string-matched in `AICore._is_emergency` *before* any LLM call. The web UI e-stop button calls the `/estop` FastAPI endpoint → `ROS2Publisher.emergency_stop()` directly, bypassing the BT engine.
...
- Standard library first; hard deps are `pyyaml`, `fastapi`, `uvicorn`, `numpy`, `scipy`, `soundfile`, `sounddevice`. STT/TTS backends (`faster-whisper`, `vosk`, `moonshine`, `piper-tts`, `kokoro`) are optional — install whichever you need. `gradio` is **gone**; `pyttsx3` is legacy. HTTP calls to Ollama use `urllib.request`, not `requests`.
- Lazy-import heavy optional deps inside methods (`rclpy`, `faster_whisper`, `vosk`, `piper`, `kokoro`). The package must import on Windows without any of them.
...
- Do not use emojis in Python code unless rendering to the web UI (icons in `_walk_tree` are OK). Terminal output should use ASCII box-drawing or plain text.
...
- **Read the whole file before you change it.** `ai_core.py` is 347 lines; `bt_engine.py` is 412; `app.py` is 1088; `stt_test.py` is 651. For the larger files, read before proposing changes.
...
- **Phone mic over LAN needs HTTPS.** Safari and Chrome block mic access on plain `http://<ip>:7860` from non-localhost origins. Workaround for demo day TBD — likely an HTTPS reverse proxy (nginx/caddy) or `ngrok`.
- **No tests/ directory yet.** I should add one; low priority until after demo day.
- **`evaluate_bt.py` is in `growmate-bt/evaluate_bt.py`** (the legacy standalone tree, not yet ported into `src/growmate_voice/`). When I ask to "run the eval" we need to copy it across and update imports to the ROS2 package structure.
- **The confirmation flow in the FastAPI app re-publishes commands but doesn't re-run safety nodes.** This is a small correctness gap I should close eventually — it means `confirm` only gates broad-scope actions at the *intent* level, not re-validating bounds after the user says yes.
